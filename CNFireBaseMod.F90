...
#include "shr_assert.h"
  ...
  type, extends(cnfire_method_type) :: cnfire_base_type
    private
      ...
      real(r8), public, pointer :: forc_bugfed(:)        ! Burned area from GFED
      ...
      type(shr_strdata_type) :: sdat_bugfed  ! Burned area from GFED input data stream
    ...
    contains
      ...
      procedure, private :: bugfed_init   ! position datasets for Burned area from GFED
      procedure, private :: bugfed_interp ! interpolates between two years of Burned area from GFED file data
...
contains
  ...
  subroutine CNFireInit( this, bounds, NLFilename )
    ...
    if ( this%need_lightning_and_popdens ) then
       ! Allocate BUGFED forcing data
       allocate( this%forc_bugfed(bounds%begg:bounds%endg) )
       this%forc_bugfed(bounds%begg:) = nan
       ...
       call this%bugfed_init(bounds, NLFilename)
       call this%bugfed_interp(bounds)
    end if
  ...
  !-----------------------------------------------------------------------
  subroutine bugfed_init( this, bounds, NLFilename )
  !
  ! !DESCRIPTION:
  !
  ! Initialize data stream information for BUGFED.
  !
  ! !USES:
  use clm_varctl       , only : inst_name
  use clm_time_manager , only : get_calendar
  ...
  !
  ! !LOCAL VARIABLES:
  integer            :: stream_year_first_bugfed ! first year in BUGFED stream to use
  integer            :: stream_year_last_bugfed   ! last year in BUGFED stream to use
  integer            :: model_year_align_bugfed   ! align stream_year_first_bugfed with 
  integer            :: nu_nml                     ! unit for namelist file
  integer            :: nml_error                  ! namelist i/o error flag
  type(mct_ggrid)    :: dom_clm                    ! domain information 
  character(len=CL)  :: stream_fldFileName_bugfed ! BUGFED stream filename to read
  character(len=CL)  :: bugfedmapalgo = 'bilinear'! Mapping alogrithm
  character(*), parameter :: subName = "('bugfeddyn_init')"
  character(*), parameter :: F00 = "('(bugfeddyn_init) ',4a)"
  !-----------------------------------------------------------------------

   namelist /bugfeds/         &
        stream_year_first_bugfed,  &
        stream_year_last_bugfed,   &
        model_year_align_bugfed,   &
        bugfedmapalgo,             &
        stream_fldFileName_bugfed

   ! Default values for namelist
    stream_year_first_bugfed  =       ! first year in stream to use
    stream_year_last_bugfed   =       ! last  year in stream to use
    model_year_align_bugfed   =       ! align stream_year_first_bugfed with this model year
    stream_fldFileName_bugfed = ''

   ! Read bugfeds namelist
   if (masterproc) then
      nu_nml = getavu()
      open( nu_nml, file=trim(NLFilename), status='old', iostat=nml_error )
      call find_nlgroup_name(nu_nml, 'bugfeds', status=nml_error)
      if (nml_error == 0) then
         read(nu_nml, nml=bugfeds,iostat=nml_error)
         if (nml_error /= 0) then
            call endrun(msg='ERROR reading bugfeds namelist'//errMsg(sourcefile, __LINE__))
         end if
      end if
      close(nu_nml)
      call relavu( nu_nml )
   ...
   call shr_mpi_bcast(stream_year_first_bugfed, mpicom)
   call shr_mpi_bcast(stream_year_last_bugfed, mpicom)
   call shr_mpi_bcast(model_year_align_bugfed, mpicom)
   call shr_mpi_bcast(stream_fldFileName_bugfed, mpicom)

   if (masterproc) then
      write(iulog,*) ' '
      write(iulog,*) 'bugfed settings:'
      write(iulog,*) '  stream_year_first_bugfed  = ',stream_year_first_bugfed  
      write(iulog,*) '  stream_year_last_bugfed   = ',stream_year_last_bugfed   
      write(iulog,*) '  model_year_align_bugfed   = ',model_year_align_bugfed   
      write(iulog,*) '  stream_fldFileName_bugfed = ',stream_fldFileName_bugfed
      write(iulog,*) ' '
   endif

   call clm_domain_mct (bounds, dom_clm)

   call shr_strdata_create(this%sdat_bugfed,name="clmbugfed",  &
        pio_subsystem=pio_subsystem,                  & 
        pio_iotype=shr_pio_getiotype(inst_name),      &
        mpicom=mpicom, compid=comp_id,                &
        gsmap=gsmap_lnd_gdc2glo, ggrid=dom_clm,       &
        nxg=ldomain%ni, nyg=ldomain%nj,               &
        yearFirst=stream_year_first_bugfed,          &
        yearLast=stream_year_last_bugfed,            &
        yearAlign=model_year_align_bugfed,           &
        offset=0,                                     &
        domFilePath='',                               &
        domFileName=trim(stream_fldFileName_bugfed), &
        domTvarName='time',                           &
        domXvarName='lon' ,                           &
        domYvarName='lat' ,                           &  
        domAreaName='area',                           &
        domMaskName='mask',                           &
        filePath='',                                  &
        filename=(/trim(stream_fldFileName_bugfed)/),&
        fldListFile='bugfed',                           &
        fldListModel='bugfed',                          &
        fillalgo='none',                              &
        mapalgo=bugfedmapalgo,                       &
        calendar=get_calendar(),                      &
        taxmode='cycle'                            )

   if (masterproc) then
      call shr_strdata_print(this%sdat_bugfed,'BUGFED data')
   endif

   ! Add history fields
   call hist_addfld1d (fname='BUGFED', units='km^2/hr',  &
         avgflag='A', long_name='BUGFED',        &
         ptr_lnd=this%forc_bugfed, default='inactive')

  end subroutine bugfed_init
  
  !-----------------------------------------------------------------------
  subroutine bugfed_interp(this, bounds )
  !
  ! !DESCRIPTION:
  ! Interpolate data stream information for BUGFED.
  !
  ! !USES:
  use clm_time_manager, only : get_curr_date
  !
  ...
  !-----------------------------------------------------------------------
   ...
   call shr_strdata_advance(this%sdat_bugfed, mcdate, sec, mpicom, 'bugfeddyn')

   ig = 0
   do g = bounds%begg,bounds%endg
      ig = ig+1
      this%forc_bugfed(g) = this%sdat_bugfed%avs(1)%rAttr(1,ig)
   end do
   
  end subroutine bugfed_interp
